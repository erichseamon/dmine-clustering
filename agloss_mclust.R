...
agloss-unsupervised <- function(year, crop, unit) {
  ...
  library(reshape2) ; damage_loss <- dcast(damage, State + County + Year + Commodity ~ Damagecause, value.var = c(unit), sum)
  ...
  damage_loss <- subset(damage_loss, Commodity == crop)
  damage_loss <- subset(damage_loss, Year == year)
